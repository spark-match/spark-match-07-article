...
prototipo actual, el agente invoca la herramienta de afinidad RIASEC sobre un
cat\'alogo piloto de carreras y, por separado, una herramienta de
b\'usqueda (\texttt{search\_programs}) que ya lo conecta a datos reales
derivados de \texttt{features.csv} (Secci\'on~\ref{sec:model-data}); lo que
falta del nodo derecho de la figura (marcado en rojo) no es la conexi\'on al
dato en s\'i, sino el \textbf{motor de scoring} que combine los cinco
criterios en un puntaje ponderado \'unico --- hoy el agente presenta afinidad
y datos reales lado a lado para que el LLM razone sobre ambos, pero no existe
a\'un esa funci\'on de combinaci\'on (Secci\'on~\ref{sec:results}). La base de datos
