...
La evaluaci\'on se apoya en un dataset real de \textbf{29 casos}
...
El dise\~no del motor de scoring completo (Secci\'on~\ref{sec:implementation})
permite, a futuro, comparar el enfoque propuesto (pesos din\'amicos inferidos
por el LLM sobre los cinco criterios) frente a alternativas m\'as simples,
como pesos est\'aticos iguales o el ordenamiento por un solo criterio (p.ej.
solo ingreso). Hoy esa comparaci\'on todav\'ia no es posible de forma
completa: solo el criterio de afinidad RIASEC est\'a integrado, as\'i que la
``inferencia din\'amica de pesos'' se limita, por ahora, a ese \'unico
criterio.
% TODO (equipo): completar la tabla comparativa (Recall@K, latencia, costo) con
% los resultados medidos.
% WARNING -- REVISAR SI ESTO ES CIERTO
\begin{table}[H]
\centering
\begin{tabular}{@{}lcl@{}}
\toprule
